...
static ssize_t ioctl_handler(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    if (cmd == 0x13370000) { // ALLOC
        ...
        long long unsigned int mem = kmem_cache_alloc(my_cache, GFP_KERNEL);
        printk("[challenge] mem = %llu", mem);
        my_buf = mem;
        if (put_user(my_buf, (unsigned long long __user *)argp)) {
            mutex_unlock(&var_mutex);
            return -EFAULT;
        }
...
static int __init challenge_init(void) {
    ...
    my_cache = kmem_cache_create("challenge", 0x1000, 0, SLAB_HWCACHE_ALIGN, NULL);
